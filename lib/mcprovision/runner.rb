  ...
  class Runner
    ...
    def provision(node)
      ...
      unless node.has_cert?
        @master.clean_cert(node_inventory[:facts].include?('fqdn') ? node_inventory[:facts]['fqdn'] : node.hostname.downcase) if @config.settings["steps"]["clean_node_certname"]

        node.send_csr if @config.settings["steps"]["send_node_csr"]

        @master.sign(node_inventory[:facts].include?('fqdn') ? node_inventory[:facts]['fqdn'] : node.hostname.downcase) if @config.settings["steps"]["sign_node_csr"]
      else
